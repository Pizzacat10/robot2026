...
public class SwerveDriveHelper {
    ...
    public static ChassisSpeeds updateChassisSpeeds(ChassisSpeeds rawChassisSpeeds, BooleanSupplier lowPower, DriveMode mode) {
        switch (mode) {
            ...
            case NewDriver:
                rawChassisSpeeds.vxMetersPerSecond *= 0.7 * (lowPower.getAsBoolean() ? 0.5 : 1);
                rawChassisSpeeds.vyMetersPerSecond *= 0.7 * (lowPower.getAsBoolean() ? 0.5 : 1);
                rawChassisSpeeds.omegaRadiansPerSecond *= 0.3 * (lowPower.getAsBoolean() ? 0.5 : 1);
                return rawChassisSpeeds;
            case Raw:
